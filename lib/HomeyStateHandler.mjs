...
export default class HomeyStateHandler extends EventEmitter {
    ...
    async _updateSystemData() {
        try {
            const systemInfo = await this._api.system.getInfo({ timeout: 10000 }); // Timeout set to 10 seconds
    ...
    _updateMemoryData() {
        ...
        if (systemInfo && systemInfo.freemem && systemInfo.totalmem && systemInfo.types) {
            try {
                // Log the fetched systemInfo for debugging purposes
                this.log('_updateMemoryData using stored systemInfo:', JSON.stringify(systemInfo));
    ...
    async _updateStorageData() {
        if (this._appMetrics) {
            try {
                const storageInfo = await this._api.system.getStorageInfo({ timeout: 10000 }); // Timeout set to 10 seconds
