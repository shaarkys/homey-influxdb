...
const _formatMeasurementName = (event, options) => {
    ...
    return (
        (!!options && options.measurementPrefix && options.measurementPrefix.length > 0
            ? _formatKey(options.measurementPrefix)
            : '') + measurementName
    );
};
...
const _formatFieldValue = (value, capability, options) => {
    ...
    if (capability && capability.units === '%') {
        switch (options?.percentageScale) {
            case 'int':
...
const fromCapability = (capability, options) => {
    ...
    const valueFormatted = _formatFieldValue(capability.value, capability.capability, options); // Pass 'options'
    ...
    return {
        ...
        timestamp: capability.ts ?? new Date()
    };
};


const fromValue = (measurement, value, options) => {
    const valueFormatted = _formatFieldValue(value, null, options); // Pass 'options' and 'capability' as null
